    ...
    <main className="flex-1 px-3 sm:px-4 md:px-8 py-4 sm:py-6 md:py-8 overflow-y-auto">
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        ...
        <section className="bg-white dark:bg-slate-900 p-3 sm:p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 md:gap-8">
            <div className="relative flex-shrink-0">
              <div className="w-20 sm:w-24 md:w-32 h-20 sm:h-24 md:h-32 rounded-full ring-4 ring-primary/20 overflow-hidden bg-slate-200 dark:bg-slate-700 shadow-lg">
                {avatarUrl ? (
            ...
            <div className="flex-1 text-center sm:text-left space-y-1.5 sm:space-y-2">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm">PWD ID: <span className="font-bold text-slate-700 dark:text-slate-300">{profile?.pwd_number}</span></p>
              <p className="text-slate-500 dark:text-slate-400 text-xs sm:text-sm">{profile?.email}</p>
              <div className="flex flex-wrap justify-center sm:justify-start gap-2 pt-1">
                {profile?.disability_type && (
                  <span className="inline-flex items-center gap-1 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-primary/10 text-primary text-[11px] sm:text-xs font-bold">
                    <span className="material-symbols-outlined text-xs sm:text-sm">accessibility</span>
                    {profile.disability_type}
                  </span>
                )}
                <span className="inline-flex items-center gap-1 px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-emerald-100 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-[11px] sm:text-xs font-bold">
        ...
        {activeSection === "personal" && (
          <section className="bg-white dark:bg-slate-900 p-3 sm:p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4 sm:space-y-5 md:space-y-6">
            <div className="flex items-center gap-2 sm:gap-3 px-2 border-l-4 border-primary">
              <h3 className="text-slate-900 dark:text-slate-50 text-lg sm:text-xl font-bold">PWD Information</h3>
              <span className="ml-auto text-[11px] sm:text-xs text-slate-400 dark:text-slate-500 font-semibold flex items-center gap-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
              {[
                ...
                <div key={item.label} className="flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3 md:p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
                  <div className="w-8 sm:w-9 h-8 sm:h-9 rounded-lg bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                    <span className="material-symbols-outlined text-primary text-sm md:text-base">{item.icon}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] sm:text-[11px] md:text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">{item.label}</p>
                    <p className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200 mt-0.5 break-words">
              ...
              {/* Address — full width */}
              <div className="flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3 md:p-4 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50 md:col-span-2">
                <div className="w-8 sm:w-9 h-8 sm:h-9 rounded-lg bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                  <span className="material-symbols-outlined text-primary text-sm md:text-base">location_on</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] sm:text-[11px] md:text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wide">Address</p>
                  <p className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200 mt-0.5 break-words">
            ...
            {/* Info note */}
            <div className="flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3 md:p-4 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-700/30">
              <span className="material-symbols-outlined text-blue-500 text-lg md:text-xl mt-0.5">info</span>
        ...
        {activeSection === "account" && (
          <section className="bg-white dark:bg-slate-900 p-3 sm:p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4 sm:space-y-5 md:space-y-6">
            <div className="flex items-center gap-2 sm:gap-3 px-2 border-l-4 border-primary">
              <h3 className="text-slate-900 dark:text-slate-50 text-lg sm:text-xl font-bold">Account Information</h3>
            </div>
            ...
            <div className="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
              <button onClick={handleUpdateAccount} disabled={saving}
                className="flex items-center gap-2 px-5 sm:px-6 md:px-8 h-9 sm:h-10 md:h-12 bg-primary hover:bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-primary/20 transition-all active:scale-95 disabled:opacity-60 text-xs sm:text-sm">
        ...
        {activeSection === "password" && (
          <section className="bg-white dark:bg-slate-900 p-3 sm:p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-4 sm:space-y-5 md:space-y-6">
            <div className="flex items-center gap-2 sm:gap-3 px-2 border-l-4 border-amber-500">
              <h3 className="text-slate-900 dark:text-slate-50 text-lg sm:text-xl font-bold">Change Password</h3>
            </div>
            <div className="p-2.5 sm:p-3 md:p-4 rounded-xl bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-700/30 flex items-start gap-2 sm:gap-3">
            ...
            <div className="space-y-3 sm:space-y-4 md:space-y-5">
              {[
                ...
                <div key={item.key} className="space-y-1.5 sm:space-y-2">
                  <label className="text-xs sm:text-sm font-bold text-slate-700 dark:text-slate-300">{item.label}</label>
                  <div className="relative">
                    ...
                    <input type={showPw[item.key] ? "text" : "password"} value={pwForm[item.field]}
                      ...
                      className="w-full h-9 sm:h-10 md:h-12 pl-11 pr-14 rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white text-xs sm:text-sm focus:border-primary focus:ring-4 focus:ring-primary/20 transition-all outline-none"
                      placeholder={item.placeholder} />
                    <button type="button" onClick={() => setShowPw((p) => ({ ...p, [item.key]: !p[item.key] }))}
                      className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors text-xs sm:text-sm">
                      <span className="material-symbols-outlined text-sm md:text-base">{showPw[item.key] ? "visibility_off" : "visibility"}</span>
                    </button>
                  </div>
            ...
            <div className="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
              <button onClick={handleChangePassword} disabled={saving}
                className="flex items-center gap-2 px-5 sm:px-6 md:px-8 h-9 sm:h-10 md:h-12 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white rounded-xl font-bold shadow-lg shadow-amber-500/20 transition-all active:scale-95 disabled:opacity-60 text-xs sm:text-sm">
      ...
      {toast && (
        <div className={`fixed bottom-4 sm:bottom-6 right-4 sm:right-6 flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2.5 sm:py-4 rounded-2xl shadow-2xl text-xs sm:text-sm font-bold text-white z-50 ${toastType === "success" ? "bg-gradient-to-r from-emerald-500 to-emerald-600" : "bg-gradient-to-r from-red-500 to-red-600"}`}>
          <span className="material-symbols-outlined text-base sm:text-lg md:text-xl">{toastType === "success" ? "check_circle" : "error"}</span>
          {toast}
        </div>
      )}
    </main>
